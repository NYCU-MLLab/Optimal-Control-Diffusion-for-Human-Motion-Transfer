...
from hamer_model.hand_net import HandNet
...
def parse_args():
    ...
    #####
    parser.add_argument(
        "--handnet_model_name_or_path",
        type=str,
        default=None,
        help="Path to pretrained handnet model",
    )
    #####
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    #####
    hand_net = HandNet(noise_latent_channels=unet.config.block_out_channels[0])
    #####
    ...
    if args.posenet_model_name_or_path is not None and args.handnet_model_name_or_path is not None and args.face_encoder_model_name_or_path is not None and args.unet_model_name_or_path is not None:
        print("Loading existing posenet weights, handnet weights, face_encoder weights and unet weights.")
        if args.posenet_model_name_or_path.endswith(".pth"):
            ...
            print(1/0)
        #####
        if args.handnet_model_name_or_path.endswith(".pth"):
            hand_net_state_dict = torch.load(args.handnet_model_name_or_path, map_location="cpu")
            hand_net.load_state_dict(hand_net_state_dict, strict=True)
            print("successfully loading the weights of hand_net")
        else:
            print("posenet weights loading fail")
            print(1/0)
        #####
    ...
    #####
    validation_hand_images = load_images_from_folder(args.validation_hand_images, width=args.width, height=args.height)
    #####
    ...
    video_frames = pipeline(
        ...
        #####
        image_hand = validation_hand_images,
        #####
        height=args.height,
        width=args.width,
