...
class LargeScaleAnimationVideos(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        hands_path = osp.join(self.video_files[idx], "hands")
        face_masks_path = osp.join(self.video_files[idx], "faces")
        hand_masks_path = osp.join(self.video_files[idx], "hand_mask")
        ...
        for index in batch_index:
            ...
            base_name = os.path.splitext(os.path.basename(tgt_img_path))[0]
            file_name = base_name + '.png'
            pose_path = osp.join(poses_path, file_name)
            face_mask_path = osp.join(face_masks_path, file_name)
            hand_path = osp.join(hands_path, file_name)
            hand_mask_path = osp.join(hand_masks_path, file_name)
            ...
            try:
                tgt_hand_mask = Image.open(hand_mask_path)
                tgt_hand_mask = tgt_hand_mask.resize((self.width, self.height))
                tgt_hand_mask = torch.from_numpy(np.array(tgt_hand_mask)).float()
                tgt_hand_mask = tgt_hand_mask / 255
            except Exception as e:
                print(f"Fail loading the hand masks: {hand_mask_path}")
                tgt_hand_mask = torch.ones(self.height, self.width, 1)
            tgt_hand_masks_list.append(tgt_hand_mask)
            ...
            try:
                ...
                hand = hand.resize((self.width, self.height))
                hand = torch.from_numpy(np.array(hand)).float()
                hand = hand / 127.5 - 1
            except Exception as e:
                print(f"Fail loading the hands: {hand_path}")
                hand = torch.zeros_like(reference_pil_image)
            hand_pil_image_list.append(hand)
        ...
        hand_pil_image_list = torch.stack(hand_pil_image_list, dim=0)
        hand_pil_image_list = rearrange(hand_pil_image_list, "f h w c -> f c h w")
        tgt_hand_masks_list = torch.stack(tgt_hand_masks_list, dim=0)
        tgt_hand_masks_list = torch.unsqueeze(tgt_hand_masks_list, dim=-1)
        tgt_hand_masks_list = rearrange(tgt_hand_masks_list, "f h w c -> f c h w")
