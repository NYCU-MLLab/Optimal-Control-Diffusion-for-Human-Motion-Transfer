...
class ValidationAnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
            ...
            image_hand: Union[torch.FloatTensor],
            height: int = 576,
            width: int = 1024,
        ...
        for hand in image_hand:
            hand = hand.resize((width, height))
        ...
        with self.progress_bar(total=len(timesteps) * len(indices)) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for idx in indices:
                    ...
                    hand_latents = self.hand_net(hand_pil_image_list[idx].to(device))
